...
const createRepositoryStatusCode = (responseText, statusCode, repositoryName) => {
  switch (statusCode) {
    case 304:
      ...
      $('#error_info').text(`<span id="unlinkRepository"> Error creating ${repositoryName} - Unable to modify repository. Try again later! </span>`,);
      $('#error_info').show();
      break;
    ...
    case 400:
      ...
      $('#unlinkRepository').hide();
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').text(`<span id="unlinkRepository"> Error creating ${repositoryName} - Bad POST request, make sure you're not overriding any existing scripts. </span>`,);
      $('#error_info').show();
      break;
    ...
    case 401:
      ...
      $('#unlinkRepository').hide();
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').text(`<span id="unlinkRepository"> Error creating ${repositoryName} - Unauthorized access to repo. Try again later! </span>`,);
      $('#error_info').show();
      break;
    ...
    case 403:
      ...
      $('#unlinkRepository').hide();
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').text(`<span id="unlinkRepository"> Error creating ${repositoryName} - Forbidden access to repository. Try again later! </span>`,);
      $('#error_info').show();
      break;
    ...
    case 422:
      ...
      $('#unlinkRepository').hide();
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').text(`<span id="unlinkRepository"> Error creating ${repositoryName} - Unprocessable Entity. Repository may have already been created. Try Linking this repository instead. </span>`,);
      $('#error_info').show();
      break;
    ...
    default:
      chrome.storage.local.set({ current_phase: 'solve_and_push' }, () => {
        $('#error_info').hide();
        $('#success_acknowledgement').html(`<span id="unlinkRepository"> Successfully created <a target="blank" href="${responseText.html_url}">${repositoryName}</a>. Start solving on <a href="https://www.geeksforgeeks.org/explore">GeeksforGeeks</a> now! </span>`,);
...
const createRepository = (accessToken, repositoryName) => {
  ...
  let repositoryInit = {
    ...
    private: true,
    auto_init: true,
    description: 'This repository contains GFG codes organised in a proper way to maintain streak.',
  };
...
const linkRepoStatusCode = (statusCode, repositoryName) => {
  ...
  switch (statusCode) {
    case 301:
      ...
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').html(
        `<span id="unlinkRepository"> Error linking  <a target="blank" class="" href="${`https://github.com/${repositoryName}`}">${repositoryName}</a> to 'GfG To GitHub'. <br> This repository has been moved permenantly. Try creating a new one. </span> `,
      );
      $('#error_info').show();
      break;
    ...
    case 403:
      ...
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').html(
        `<span id="unlinkRepository"> Error linking <a target="blank" href="${`https://github.com/${repositoryName}`}">${repositoryName}</a> to 'GfG To GitHub'. <br> Forbidden action. Please make sure you have the right access to this repository. </span>`,
      );
      $('#error_info').show();
      break;
    ...
    case 404:
      ...
      $('#unlinkRepository').hide();
      $('#error_info').attr("style","display: block; line-height: 1;");
      $('#error_info').html(`<span id="unlinkRepository"> Error linking <a target="blank" href="${`https://github.com/${repositoryName}`}">${repositoryName}</a> to 'GfG To GitHub'. <br> Resource not found. Make sure you enter the right repository name. </span>`,);
      $('#error_info').show();
      ...
      if (xhttp.status === 200) {
        if (!linkFlag) {
          ...
          chrome.storage.local.set(
            ...
            () => {
              $('#error_info').hide();
              $('#success_acknowledgement').html(`<span id="unlinkRepository"> Successfully linked <a target="blank" href="${responseText.html_url}">${repositoryName}</a> to 'GfG To GitHub'. Start solving on <a href="https://www.geeksforgeeks.org/explore">GeeksforGeeks</a>&nbsp; now! </span>`,);
              $('#success_acknowledgement').show();
              $('#unlinkRepository').show();
            },
          );
